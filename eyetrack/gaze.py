...
class CalibrationSession:
    ...
    current_point_samples: list[tuple[float, float]] = field(default_factory=list)
...
def fit_five_point_affine(
    ...
    solution, _, _, _ = np.linalg.lstsq(source, target, rcond=None)
...
def predict_screen_point(
    ...
    uv = model.matrix @ np.array([feature_dx, feature_dy, 1.0], dtype=np.float64)
    u = float(uv[0])
    v = float(uv[1])
...
def advance_calibration_session(
    ...
    if feature_valid and feature_dx is not None and feature_dy is not None:
        session.current_point_samples.append((float(feature_dx), float(feature_dy)))
    ...
    median_dx = float(np.median(point_samples[:, 0]))
    median_dy = float(np.median(point_samples[:, 1]))
    session.collected_samples.append(
        CalibrationSample(
            point_name=point.name,
            target_u=point.u,
            target_v=point.v,
            feature_dx=median_dx,
            feature_dy=median_dy,
        )
    )
    ...
    if session.point_index >= len(session.points):
        try:
            session.model = fit_five_point_affine(session.collected_samples, expected_point_count=len(session.points))
