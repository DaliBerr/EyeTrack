...
def detect_bad_frames(
    ...
    param area_ratio_min: minimumthreshold
    param area_ratio_max: maximumthreshold
    ...
    param anomaly_display_pad: anomaly
    return: anomaly dict
    """
...
def split_core_bad_runs(core_bad: np.ndarray, max_interp_gap: int) -> Dict[str, np.ndarray]:
    """
    summary: anomalymask anomaly anomaly
    param core_bad: anomaly mask
    param max_interp_gap: maximum anomaly
    return: anomaly, anomaly dict
    """
...
def interpolate_short_runs(series: np.ndarray, short_bad_mask: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    param short_bad_mask: anomaly mask
...
def apply_long_gap_mask(series: np.ndarray, long_bad_mask: np.ndarray) -> np.ndarray:
    ...
    summary: anomaly NaN
    param series: inputsequence
    param long_bad_mask: anomaly mask
...
def build_clean_series(
    ...
    param df: inputsequence DataFrame
    param core_bad: anomalymask
    param max_interp_gap: maximumanomaly
    param smooth_gap_pad: anomaly smoothing
    param median_window: window
    param ema_alpha: EMA smoothing
    return: dict
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--anomaly_display_pad", type=int, default=1, help=" anomaly ")

    parser.add_argument("--max_interp_gap", type=int, default=2, help=" maximum anomaly ")
    parser.add_argument("--smooth_gap_pad", type=int, default=1, help=" anomaly smoothing ")
